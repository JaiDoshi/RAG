...
class RAGModel:
    ...
    def generate_answer(self, query: str, search_results: List[Dict]) -> str:
        ...
        """
        messages_compressor = [
            {"role": "system", "content": """You are a Retrieval Augmented model. Given the following question and context, extract any part of the context *AS IS* that is relevant to answer the question. If none of the context is relevant return NO_OUTPUT. 
             """},
            {"role": "user", "content": prompt_template},
        ] 

        prompt_compressor = self.generation_pipe2.tokenizer.apply_chat_template(
                messages_compressor, 
                tokenize=False, 
                add_generation_prompt=True
        )

        terminators_compressor = [
            self.generation_pipe2.tokenizer.eos_token_id,
            self.generation_pipe2.tokenizer.convert_tokens_to_ids("<|eot_id|>")
        ]


        hfp = HuggingFacePipeline(pipeline=self.generation_pipe2)
        compressor = LLMChainExtractor.from_llm(hfp)
